...
This chunk uses `semPower::semPower.aPriori()` to compute the minimum sample size required for an a priori power analysis based on RMSEA, given the target \(\alpha\), desired power, and model degrees of freedom [@moshagen2024].
...
This chunk runs an **a priori analytical power analysis** with `semPower.powerLav()`. The function uses the population model (`modelPop`) and the null analysis model (`modelH0`) to determine the minimum sample size required to detect their discrepancy with the target alpha and power [@moshagen2024; @jobst2023].

Because no `modelH1` is specified, the comparison is made against the **saturated model**, so the test targets global model misfit [@moshagen2024; @jobst2023].
...
This chunk generates a model-free power curve showing how statistical power changes as a function of sample size \(N\) for a fixed effect defined as \(\mathrm{RMSEA} = 0.05\), given the chosen significance level \(\alpha\) and the model degrees of freedom \(df\) [@moshagen2024; @jobst2023].

In `semPower`, this is an analytical power plot for a noncentrality-based effect measure, so the curve summarizes the expected power across a range of sample sizes while holding the effect size and test conditions constant [@moshagen2024; @jobst2023].
