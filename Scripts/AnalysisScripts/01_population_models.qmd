...
param_transp$parameters <- paste0("`", param_transp$parameters, "`")
...
get_metrics <- function(model_name) {
  ...
  # Remove display backticks added in the previous chunk.
  params <- gsub("`", "", param_transp$parameters, fixed = TRUE)
  ...
  left_right <- strsplit(params[is_cov], "~~", fixed = TRUE)
  is_diff <- vapply(left_right, function(x) length(x) == 2 && x[1] != x[2], logical(1))
  ...
  left_right_res <- strsplit(params[is_res], "~~", fixed = TRUE)
  is_same_res <- vapply(left_right_res, function(x) length(x) == 2 && x[1] == x[2], logical(1))
